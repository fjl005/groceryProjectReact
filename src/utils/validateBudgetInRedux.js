export const validateBudgetInRedux = (input) => {
    let errors = '';
    ...
    if ((input < 1) || (!(regExGreaterThanZero.exec(input)))) {
        errors = 'You must enter a budget greater than $1';
    }

    // If commas exist, then check that it's used properly (with 1-3 digits before it and exactly 3 digits after it).
    if ((regExCommaPresence.exec(input)) && (!(regExCommaUse.exec(input)))) {
        ...
        if ((regExPeriodPresence.exec(input)) && (!(regExPeriodUse.exec(input)))) {
            errors = 'Only valid numerical inputs are allowed (fix the periods!)';
        } else {
            errors = 'Only valid numerical inputs are allowed (fix the commas!)';
        }
    }
    ...
    if (regExDigitsOnly.exec(input)) {
        errors = 'Only valid numerical inputs are allowed';
    }
    return errors;
}
